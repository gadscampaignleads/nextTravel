...
function Booking() {
    ...
    const { flight: flightQuery, location: locationQuery } = router.query;
    ...
    console.log("booking pageggggggggg")
    // console.log(router.query)
    // console.log(flight) 
    // console.log(location)

    // Parse flight and location data from query
    const flight = flightQuery ? JSON.parse(flightQuery) : null;
    const location = locationQuery ? JSON.parse(locationQuery) : null;
    ...
    useEffect(() => {
        if (!flight) {
            toast.error("Flight data is missing!");
            router.push('/'); // Redirect to home if flight data is missing
        }
    }, [flight, router]);

    const grandTotal = Number(convertPrice(flight?.price_dropdown?.base_fare * adt, currency)) +
        Number(convertPrice(flight?.price_dropdown?.base_fare * ift, currency)) +
        Number(convertPrice(flight?.price_dropdown?.base_fare * chd, currency));
    const serviceFee = Number(convertPrice(15, currency));
    const totalAmount = grandTotal + serviceFee;
    ...
    const validateForm = () => {
        const { email, cardNumber, cardHolderName } = formData;
    ...
    const finalData = { ...formData, flight, totalAmount, adt, ift, chd, currency };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (validateForm()) {
                ...
                router.push("/thankyou", { state: { billData: data } });
            }
        } catch (error) {
                                ...
                                <div className="col-12 flex_me d-flex d-lg-none">
                                    ...
                                    <div className='price_summ' onClick={() => {
                                        ...
                                        <p className='price_tit'>
                                            ...
                                            <span className='color_redd'>{currency}</span> {totalAmount}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div className="boking_header" >
                                    <div className="booking_main">
                                        <p className='booking_title'>
                                            ...
                                            <span>(Please provide the Following details  to complete your booking)</span></p>
                                    ...
                                    <div className="booking_main booking_main1">
                                        <p className='booking_title'>
                                            <div className="book_semi1">
                                                ...
                                                Contact Information </div>  <span>(Enter your email and phone number and we will send you a booking)</span></p>
                                        <div className="mt-4 booking_main_m">
                                            <div className="row form_booking_f">
                                                <div className="col-6">
                                                    <div className="form_booking">
